...
def run_server():
    ...
    if topic_check.topic_exists("cta.trains.monitor.turnstile.summarized") is False:
        logger.fatal(
            "Ensure that the KSQL Command has run successfully before running the web server!"
        )
        exit(1)
    if topic_check.topic_exists("cta.trains.monitor.stations.transformed") is False:
        logger.fatal(
